...
internal class NetherCaveCarver : Carver
{
    ...
    protected void CarveNetherCaves(int chunkX, int chunkZ, byte[] blocks, double x, double y, double z, float tunnelRadius, float yaw, float pitch, int tunnelStep, int tunnelLength, double verticalScale, JavaRandom caveRand)
    {
        ...
        for (bool isLargeRoom = caveRand.NextInt(6) == 0; tunnelStep < tunnelLength; ++tunnelStep)
        {
            ...
            {
                CarveNetherCaves(chunkX, chunkZ, blocks, x, y, z, caveRand.NextFloat() * 0.5F + 0.5F, yaw - (float)Math.PI * 0.5F, pitch / 3.0F, tunnelStep, tunnelLength, 1.0D, new(Rand.NextLong()));
                CarveNetherCaves(chunkX, chunkZ, blocks, x, y, z, caveRand.NextFloat() * 0.5F + 0.5F, yaw + (float)Math.PI * 0.5F, pitch / 3.0F, tunnelStep, tunnelLength, 1.0D, new(Rand.NextLong()));
                return;
            }
    ...
    protected override void CarveCaves(World world, int chunkX, int chunkZ, int centerChunkX, int centerChunkZ, byte[] blocks)
    {
        ...
        for (int i = 0; i < numCaves; ++i)
        {
            ...
            for (int branch = 0; branch < branchCount; ++branch)
            {
                ...
                CarveNetherCaves(centerChunkX, centerChunkZ, blocks, randX, randY, randZ, tunnelRadius * 2.0F, yaw, pitch, 0, 0, 0.5D, new(Rand.NextLong()));
            }
        }
    }
}
