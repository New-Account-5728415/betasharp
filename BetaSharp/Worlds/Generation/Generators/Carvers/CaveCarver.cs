...
internal class CaveCarver : Carver
{
    ...
    protected void CarveCaves(int chunkX, int chunkZ, byte[] blocks, double offsetX, double offsetY, double offsetZ, float tunnelRadius, float yaw, float pitch, int tunnelStep, int tunnelLength, double verticalScale, JavaRandom caveRand)
    {
        ...
        int branchStep = tunnelStep < tunnelLength / 4 * 3 ? tunnelStep + caveRand.NextInt((tunnelLength - tunnelStep) / 2) + (tunnelLength - tunnelStep) / 4 : -1;
        ...
        for (bool isLargeRoom = caveRand.NextInt(6) == 0; tunnelStep < tunnelLength; ++tunnelStep)
        {
            ...
            {
                CarveCaves(chunkX, chunkZ, blocks, offsetX, offsetY, offsetZ, caveRand.NextFloat() * 0.5F + 0.5F, yaw - (float)Math.PI * 0.5F, pitch / 3.0F, tunnelStep, tunnelLength, 1.0D, new(Rand.NextLong()));
                CarveCaves(chunkX, chunkZ, blocks, offsetX, offsetY, offsetZ, caveRand.NextFloat() * 0.5F + 0.5F, yaw + (float)Math.PI * 0.5F, pitch / 3.0F, tunnelStep, tunnelLength, 1.0D, new(Rand.NextLong()));
                return;
            }
            ...
            if (isStartingPoint || caveRand.NextInt(4) != 0)
            {
                ...
                if (distX * distX + distZ * distZ > (stepsRemaining + boundRadius) * (stepsRemaining + boundRadius))
                {
                    return;
                }
    ...
    protected override void CarveCaves(World world, int chunkX, int chunkZ, int centerChunkX, int centerChunkZ, byte[] blocks)
    {
        ...
        for (int i = 0; i < numCaves; ++i)
        {
            ...
            for (int branch = 0; branch < branchCount; ++branch)
            {
                ...
                CarveCaves(centerChunkX, centerChunkZ, blocks, caveX, caveY, caveZ, tunnelRadius, yaw, pitch, 0, 0, 1.0D, new(Rand.NextLong()));
            }
        }
    }
}
